...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    // ดึงงานซ่อมและดึงข้อมูล profile ของผู้แจ้งมาด้วย (ถ้ามี)
  ...
  const saveTaskUpdate = async (taskId) => {
    ...
    if (!updates) return alert('ไม่มีการเปลี่ยนแปลง');
    const { error } = await supabase.from('repair_tasks').update(updates).eq('id', taskId);
    if (error) alert(error.message);
    else { alert('บันทึกสำเร็จ'); fetchData(); setEditTasks(prev => { const copy = { ...prev }; delete copy[taskId]; return copy; }); }
  };
  ...
  return (
    <div style={styles.body}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '25px' }}>
        ...
        <div style={styles.card}>สมาชิก<div style={{ fontSize: '24px' }}>{stats.allMembers}</div></div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        {/* 2. รายชื่อสมาชิก (ขวาล่างย้ายมานี่) */}
            ...
            <tbody>
              {members.map(m => (
                <tr key={m.id}>
                  ...
                  <td style={styles.td}><span style={{color:'#4d94ff', cursor:'pointer'}} onClick={() => {setSelectedMember(m); setIsMemberModalOpen(true);}}>แก้ไข</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* 4. รายการงานซ่อม (เพิ่มคอลัมน์บทบาท) */}
      <div style={styles.section}>
          ...
          <tbody>
            {tasks.map(t => (
              <tr key={t.id}>
                ...
                <td style={styles.td}>
                  <span 
                    style={{ color: '#00ccff', cursor: 'pointer', fontWeight: 'bold' }} 
      ...
      {/* Modal รายละเอียดผู้แจ้งและงานซ่อม */}
      {isTaskModalOpen && selectedTask && (
        <div style={styles.modalOverlay} onClick={() => setIsTaskModalOpen(false)}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <h3 style={{ color: '#00ccff', borderBottom: '1px solid #333', paddingBottom: '10px' }}>📋 ข้อมูลการแจ้งซ่อม</h3>
            
            {/* ข้อมูลผู้แจ้ง */}
            <div style={{ marginBottom: '20px' }}>
              <p style={{ color: '#ff4d4d', fontSize: '12px', fontWeight: 'bold' }}>👤 ข้อมูลผู้แจ้ง</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '14px' }}>
                ...
                <div>ID: {selectedTask.member_id || 'Guest User'}</div>
              </div>
              {selectedTask.profiles?.address && (
                <div style={{ marginTop: '10px', fontSize: '13px', color: '#aaa' }}>ที่อยู่: {selectedTask.profiles.address}</div>
              )}
            </div>

            {/* ข้อมูลอุปกรณ์ */}
            <div style={{ marginBottom: '20px', borderTop: '1px solid #333', paddingTop: '15px' }}>
              <p style={{ color: '#ff4d4d', fontSize: '12px', fontWeight: 'bold' }}>🔧 รายละเอียดอุปกรณ์</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '14px' }}>
                <div>อุปกรณ์: {selectedTask.device_type}</div>
                <div>ยี่ห้อ: {selectedTask.brand || '-'}</div>
                <div>ทะเบียน/ซีเรียล: {selectedTask.registration_number || '-'}</div>
                <div>สี: {selectedTask.color || '-'}</div>
              </div>
              <div style={{ marginTop: '10px' }}>
                <span style={{ color: '#888', fontSize: '12px' }}>อาการที่แจ้ง:</span>
                <div style={{ backgroundColor: '#222', padding: '10px', borderRadius: '5px', marginTop: '5px' }}>{selectedTask.description}</div>
              </div>
            </div>

            {/* หมายเหตุช่าง */}
            <div style={{ borderTop: '1px solid #333', paddingTop: '15px' }}>
              <p style={{ color: '#ff4d4d', fontSize: '12px', fontWeight: 'bold' }}>✍️ บันทึกเพิ่มเติมจากช่าง</p>
              <textarea 
                style={{ ...styles.input, height: '80px', marginTop: '5px' }} 
                defaultValue={selectedTask.technician_comment}
                placeholder="กรอกหมายเหตุหรือความเห็นช่าง..."
                onChange={e => handleTaskChange(selectedTask.id, 'technician_comment', e.target.value)}
              />
            </div>

            <button style={styles.btnRed} onClick={() => {saveTaskUpdate(selectedTask.id); setIsTaskModalOpen(false);}}>บันทึกข้อมูล & ปิดหน้าต่าง</button>
          </div>
        </div>
      )}
    </div>
