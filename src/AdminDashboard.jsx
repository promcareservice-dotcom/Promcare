...
const AdminDashboard = () => {
  ...
  const [newTask, setNewTask] = useState({
    member_id: '', device_type: '', brand: '', model: '', details: '', status: 'pending'
  });
  ...
  const fetchData = async () => {
    ...
    if (memberData) {
      // จัดเรียงสมาชิก: Admin > Technician > Customer
  ...
  const handleUpdateTask = async (taskId) => {
    ...
    else { alert('บันทึกการอัปเดตงานเรียบร้อยแล้ว'); fetchData(); setIsTaskModalOpen(false); setEditData({}); }
  };
  ...
  const handleCreateNewTask = async () => {
    ...
    if (error) alert('Error: ' + error.message);
      ...
      setNewTask({ member_id: '', device_type: '', brand: '', model: '', details: '', status: 'pending' });
      fetchData();
    }
  };
  ...
  const styles = {
    ...
    searchBox: { padding: '12px 20px', borderRadius: '15px', border: '1px solid #333', backgroundColor: '#111', color: '#fff', width: '300px', outline: 'none' },
    table: { width: '100%', borderCollapse: 'separate', borderSpacing: '0 10px' },
    tr: { backgroundColor: '#0a0a0a' },
    td: { padding: '20px', borderBottom: '1px solid #111' },
    input: { backgroundColor: '#1a1a1a', border: '1px solid #333', color: '#fff', padding: '12px', borderRadius: '10px', width: '100%', marginBottom: '12px', outline: 'none' },
  ...
  return (
    <div style={styles.container}>
      ...
      {activeTab === 'tasks' ? (
            ...
            <tbody>
              {filteredTasks.map(t => (
                <tr key={t.id} style={styles.tr}>
                  <td style={styles.td}><small style={{color:'#666'}}>{formatThaiDate(t.created_at)}</small></td>
                  <td style={styles.td}><strong>{t.customer_name || t.profiles?.full_name || t.guest_name}</strong><br/>{getRoleBadge(t.profiles?.role || 'guest')}</td>
                  <td style={styles.td}>
                    <div style={{color:'#00ccff', cursor:'pointer', fontWeight:'bold'}} onClick={() => { setSelectedTask(t); setIsDetailModalOpen(true); }}>{t.device_type}</div>
                    <small style={{color:'#888'}}>📍 {t.brand} {t.model}</small>
                  </td>
                  <td style={styles.td}>
                    <select style={{...styles.input, width:'auto', marginBottom:0}} value={editData[t.id]?.status || t.status} onChange={e=>setEditData({...editData, [t.id]:{...(editData[t.id]||{}), status: e.target.value}})}>
                      <option value="pending">⏳ รอรับงาน</option>
                      <option value="in_progress">⚙️ กำลังซ่อม</option>
                      <option value="completed">✅ เสร็จสิ้น</option>
                    </select>
                    <button style={{marginLeft:'10px', color:'#ff4d4d', background:'none', border:'none', cursor:'pointer'}} onClick={()=>handleUpdateTask(t.id)}>บันทึก</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <section>
          <div style={{display:'flex', justifyContent:'space-between', marginBottom:'25px'}}>
            <input style={styles.searchBox} placeholder="🔍 ค้นหาสมาชิก (ชื่อ, เมล, เบอร์)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            ...
            <tbody>
              {filteredMembers.map(m => (
                <tr key={m.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <button style={{marginRight:'15px', color:'#00ccff', background:'none', border:'none', cursor:'pointer'}} onClick={()=>{setSelectedMember(m); setIsMemberModalOpen(true)}}>แก้ไข</button>
      ...
      {/* Modal แจ้งงานซ่อมใหม่ (Admin) */}
      {isNewTaskModalOpen && (
        <div style={styles.modal} onClick={()=>setIsNewTaskModalOpen(false)}>
          <div style={styles.modalBody} onClick={e=>e.stopPropagation()}>
            <h3 style={{color:'#00ccff', marginTop:0, marginBottom:'25px'}}>🛠 เปิดใบแจ้งซ่อมใหม่ (Admin)</h3>
            <label style={{fontSize:'12px', color:'#888'}}>เลือกสมาชิกในระบบ</label>
            ...
            <input style={styles.input} placeholder="ประเภทอุปกรณ์ (เช่น โน้ตบุ๊ก, ปริ้นเตอร์)" onChange={e=>setNewTask({...newTask, device_type: e.target.value})} />
            <div style={{display:'flex', gap:'10px'}}>
            ...
            <textarea style={{...styles.input, height:'100px'}} placeholder="อาการเสียที่พบ..." onChange={e=>setNewTask({...newTask, details: e.target.value})} />
            <button style={{...styles.primaryBtn, width:'100%', padding:'15px', fontSize:'16px'}} onClick={handleCreateNewTask}>บันทึกงานซ่อมใหม่</button>
          </div>
        </div>
      )}

      {/* Modal เพิ่มสมาชิกใหม่ (พร้อม Fix Username) */}
      {isAddMemberOpen && (
        <div style={styles.modal} onClick={()=>setIsAddMemberOpen(false)}>
          <div style={styles.modalBody} onClick={e=>e.stopPropagation()}>
            <h3 style={{color:'#28a745', marginTop:0, marginBottom:'25px'}}>✨ เพิ่มสมาชิกใหม่</h3>
            ...
            <button style={{...styles.primaryBtn, width:'100%', padding:'15px', backgroundColor:'#28a745'}} onClick={handleAddMember}>บันทึกสมาชิก</button>
          </div>
        </div>
      )}
    </div>
